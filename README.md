...
> 工作区内亦保留 Y1、AR5、Piper 等历史配置，**本文示例均以 V15 → Franka 为准**。  
...
**使用 SSH（推荐）:**
```bash
git clone git@gitlab.pegasus-ai.cn:infra-embedded-software/nexus-sdk.git
cd nexus-sdk
```

**使用 HTTPS:**
```bash
git clone https://gitlab.pegasus-ai.cn/infra-embedded-software/nexus-sdk.git
cd nexus-sdk
```
...
> ```bash
> source /opt/ros/humble/setup.bash
> source ~/nexus-sdk/install/setup.bash  # 根据实际路径修改
> ```
...
如有问题，请联系：
- GitLab Issues: [nexus-sdk Issues](https://gitlab.pegasus-ai.cn/infra-embedded-software/nexus-sdk/-/issues)
