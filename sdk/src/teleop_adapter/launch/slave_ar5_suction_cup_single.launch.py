#!/usr/bin/env python3
"""
Launch file for single AR5 arm with suction cup teleop control
Loads configuration from slave_ar5_suction_cup_single.yaml and starts the driver node.
...
def generate_launch_description():
    ...
    config_file = os.path.join(pkg_share, 'config', 'slave_ar5_suction_cup_single.yaml')

    existing_ld = os.environ.get('LD_LIBRARY_PATH', '')
    ld_path = pkg_lib + ':' + existing_ld if existing_ld else pkg_lib

    ar5_suction_cup_arm_driver_node = Node(
    ...
    return LaunchDescription([
        ar5_suction_cup_arm_driver_node
    ])
