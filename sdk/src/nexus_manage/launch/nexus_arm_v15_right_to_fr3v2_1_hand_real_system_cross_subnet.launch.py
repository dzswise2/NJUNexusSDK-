#!/usr/bin/env python3
"""
跨网段启动文件 — Nexus-Arm V15 Right → Franka FR3v2.1 + Hand

通过路由器 NAT 端口转发实现跨子网 DDS 通信。
支持一台 Master（Nexus-Arm）遥控一台或多台 Slave（Franka），
多从臂时 robot_id 与 slave_registry.yaml 中 id 一致。

用法:
  # === Master 侧（Nexus-Arm 主机，只启动一次）===
  ros2 launch nexus_manage nexus_arm_v15_right_to_fr3v2_1_hand_real_system_cross_subnet.launch.py role:=master

  # === Slave 侧（每台 Franka 工控机各启动一次）===
  ros2 launch nexus_manage nexus_arm_v15_right_to_fr3v2_1_hand_real_system_cross_subnet.launch.py role:=slave robot_id:=fr3v2_1_01
...
_MASTER_NODES = [
    ...
    ('nexus_manage',     'nexus_nexus-arm_v15_right_to_fr3v2_1_hand_manage.launch.py'),
]
