...
从臂 `arm_control_node` 加载，命名空间为 `master_arm_controller`（历史命名，表示该 yaml 内的控制器参数块）。

> **注意**：文件头注释仍写 AR5 模板来源；实机前须按 FR3 关节限位与力矩上限重标定 §8 增益。
...
```bash
python3 src/robot_controller/scripts/derive_franka_gripper_mapping.py
```
...
| 夹爪主从行程比例不对 | `gripper_scaling_factor`；`derive_franka_gripper_mapping.py` |
