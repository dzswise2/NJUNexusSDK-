#!/usr/bin/env python3
"""
主启动文件 - 一键启动整个远程操控系统（Nexus-Arm V15 Right → AR5 08L Suction Cup）

用法:
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py role:=slave
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py role:=master
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py role:=slave domain_id:=42
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py role:=master network_interface:=enp3s0
  ros2 launch nexus_manage nexus_arm_v15_right_to_ar5_08l_suction_cup_real_system.launch.py role:=slave network_interface:=eth0
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    ar5_arm_launch = _include('teleop_adapter', 'slave_ar5_08l_suction_cup_single.launch.py', launch_args)
    ar5_human_data_solver_launch = _include('human_data', 'ar5_08l_suction_cup_human_data_solver.launch.py', launch_args)
    slave_controller_launch = _include('robot_controller', 'slave_single_ar5_08l_suction_cup.launch.py', launch_args)

    nexus_arm_launch = _include('teleop_adapter', 'master_nexus_single.launch.py', launch_args)
    nexus_arm_human_data_solver_launch = _include('human_data', 'nexus_arm_v15_right_human_data_solver.launch.py', launch_args)
    master_controller_launch = _include('robot_controller', 'master_single_nexus_v15_right.launch.py', launch_args)
    manager_launch = _include('nexus_manage', 'nexus_nexus-arm_v15_right_to_ar5_08l_suction_cup_manage.launch.py', launch_args)
