...
def main() -> None:
    ...
    insert_marker = "}  // namespace\n\nArmController::ArmController()"
    if insert_marker not in s:
        raise SystemExit("insert marker not found")
    s = s.replace(insert_marker, "}  // namespace\n\n" + IMPL_STRUCT + "\nArmController::ArmController()", 1)
    ...
    if cm_start < 0 or cm_end < 0:
        raise SystemExit("computeCartMotionControl block not found")
    block = s[cm_start:cm_end]
    block = re.sub(r"\bEigen::VectorXd q_target\b", "Eigen::VectorXd q_cart_ik", block, count=1)
    ...
    if jsp_start < 0 or jsp_next < 0:
        raise SystemExit("computeJointSpaceMotionPlanning block not found")
...
void ArmController::setInitialJointState(const JointState& initial_joint_state) {
    impl_->initial_joint_state_ = initial_joint_state;
}

GripperController* ArmController::getGripperController() {
    return impl_->gripper_controller_.get();
}

const Eigen::VectorXd& ArmController::getLastCartPoseError() const {
    return impl_->last_cart_pose_error_;
}

const Eigen::VectorXd& ArmController::getLastPoseEeUpdated() const {
    return impl_->last_pose_ee_updated_;
}

const Eigen::VectorXd& ArmController::getLastDesiredPose() const {
    return impl_->last_desired_pose_;
}

const Eigen::VectorXd& ArmController::getLastIkTargetPoseXyzRpy() const {
    return impl_->last_ik_target_pose_xyz_rpy_;
}

const Eigen::VectorXd& ArmController::getLastJointPosError() const {
    return impl_->last_joint_pos_error_;
}

MitControlCommand ArmController::computeCartMotionControlWithCBF(
    ...
    return computeCartMotionControlWithCBFinSimRobot(
        current_joint_state, desired_pose, gripper_target_angle, slave_external_force);
}

'''
    # Insert extra before final namespace closes
    closing = "\n}  // namespace controllers\n}  // namespace teleop_app\n"
    if closing not in s:
        raise SystemExit("final closing not found")
    s = s.replace(closing, extra + closing, 1)
